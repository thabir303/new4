...
const { GeneticAlgorithm } = require('genetic-algorithm'); // Import GeneticAlgorithm from the genetic-algorithm module
...
async function fetchDataAndSetParameters() {
    try {
        ...
        landfills.forEach(landfill => {
            const capacities = Array.isArray(landfill.capacity) ? landfill.capacity.map(capacity => capacity) : [];
...
router.get('/', async (req, res) => {
    try {
        await fetchDataAndSetParameters();
        const result = ga.run();
        res.json({ optimalSolution: result.bestIndividual, fitness: result.bestFitness });
    } catch (error) {
